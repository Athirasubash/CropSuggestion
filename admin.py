...
@admin.route('/adminviewfarmer',methods=['get','post'])
def adminviewfarmer():
	data={}
	q="SELECT * FROM `farmers` INNER JOIN login USING(login_id)"
	res=select(q)
	data['farmer']=res
	print(res)
...
def adminviewenquiry():
	...
	return render_template("adminviewenquiry.html",data=data)
