	...
	if 'submit' in request.form:
		...
		if res:
			...
			if res[0]['usertype']=="farmer":
				q="select * from farmers where login_id='%s'"%(session['lid'])
				res=select(q)
				session['name']=res[0]['first_name']
				return redirect(url_for('farmer.farmerhomes'))
			if res[0]['usertype']=="expert":
				q="select * from experts where login_id='%s'"%(session['lid'])
				res=select(q)
				session['name']=res[0]['fname']
...
def expertreg():
	if 'submit' in request.form:
		...
		qul.save(path)
		idp=request.files['id']
		path2='static/'+str(uuid.uuid4())+idp.filename 
		idp.save(path2)
		q="select * from login where username='%s' or password='%s'" %(uname,pwd)
		res=select(q)
		if res:
			...
			id=insert(q)
			q="insert into experts values(null,'%s','%s','%s','%s','%s','%s','%s')" %(id,fname,lname,phone,email,path,path2)
			insert(q)
			return redirect(url_for('public.login'))
